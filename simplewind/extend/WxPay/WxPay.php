...
class WxPay
{
    ...
    */
    ...
    public function getParameters()
    {
        ...
        // dump($this->payment['wxpay_appid']);die;
    ...
    function getPrepayId()
    {
        ...
        try {
            ...
            // dump($this->parameters["appid"].'/'.$this->parameters["mch_id"] );die;
        ...
        $response = $this->curl_data($url, $xml, 30);
        /**
...
//                're_user_name'=>$data['re_user_name']??'',
            ...
            $response = $this->curl_datas($url, $xml, 30,['is_post'=>true,'pt'=>false, 'qy' => true]);
            // dump($response);die;
            $response = json_decode(json_encode(simplexml_load_string($response, 'SimpleXMLElement', LIBXML_NOCDATA)), true);
    ...
    public function turnCash($data){
        ...
        try {
            ...
            $xml = $this->toXml($post_data);
            $response = curl_data($url, $xml, 30,['is_post'=>true,'pt'=>true,'qy' => true]);
            $response = json_decode(json_encode(simplexml_load_string($response, 'SimpleXMLElement', LIBXML_NOCDATA)), true);
            return $response;
    ...
    public function curl_datas($url, $data=[], $timeout=30, $useCert=[])
    {
        // var_dump($data);
        $ch = curl_init();
        ...
        curl_setopt($ch, CURLOPT_POST, true);
         // dump(config('SSLCERT_PATH'));die;
        if($useCert['pt'] == true){
            curl_setopt($ch,CURLOPT_SSLCERTTYPE,'PEM');
            curl_setopt($ch,CURLOPT_SSLCERT, '/mnt/zhyxcx/bushubao/simplewind/extend/WxPay/cert/apiclient_cert.pem');
            curl_setopt($ch,CURLOPT_SSLKEYTYPE,'PEM');
            curl_setopt($ch,CURLOPT_SSLKEY, '/mnt/zhyxcx/bushubao/simplewind/extend/WxPay/cert/apiclient_key.pem');
        }

        if($useCert['is_post'] == true){
            if($useCert['qy']){
                $sslcert = '/mnt/zhyxcx/bushubao/simplewind/extend/WxPay/cert/apiclient_cert.pem';
                $sslkey = '/mnt/zhyxcx/bushubao/simplewind/extend/WxPay/cert/apiclient_key.pem';
            }else{
                $sslcert = '/mnt/zhyxcx/bushubao/simplewind/extend/WxPay/cert/apiclient_cert.pem';;
                $sslkey = '=/mnt/zhyxcx/bushubao/simplewind/extend/WxPay/cert/apiclient_key.pem';
            }
