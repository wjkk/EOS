...
class SettingController extends AdminBaseController
{
    ...
    public function site()
    {
        ...
        $options['q_num'] = config('q_num');

        $options['txy_sms_appid'] = config('txy_sms_appid');
        $options['txy_sms_appkey'] = config('txy_sms_appkey');
        $options['txy_sms_smssign'] = config('txy_sms_smssign');
        $options['txy_sms_templateid'] = config('txy_sms_templateId');

        $options['rule1'] = config('rule1');
        $options['rule2'] = config('rule2');
        $options['rule3'] = config('rule3');
        $options['rule4'] = config('rule4');
        $options['rule5'] = config('rule5');
        $options['rule6'] = config('rule6');
        $options['rule7'] = config('rule7');

        $options['revive_card'] = config('revive_card');
        $options['rule'] = config('rule');



        // dump($options['q_num']);die;
        // $this->assign('zm_point', $options['zm_point']);
    ...
    public function _site()
    {
        ...
        $res =  set_config( $option );
        if( !$res ){
            $this->error('保存失败' );
        }

        $this->success('保存成功',url('site'),1 );
    }
